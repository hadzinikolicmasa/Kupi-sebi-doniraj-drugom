...
CREATE TABLE IF NOT EXISTS `kupi_sebi_doniraj_drugom`.`Korisnik` (
  ...
  UNIQUE INDEX `korisničko ime_UNIQUE` (`korisnickoime` ASC) )
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `kupi_sebi_doniraj_drugom`.`Licitacija` (
  ...
  INDEX `fk_Licitacija_Kategorija_idx` (`Kategorija_IdKategorije` ASC) ,
  INDEX `fk_Licitacija_Fondacija1_idx` (`Fondacija_idFondacija` ASC) ,
...
CREATE TABLE IF NOT EXISTS `kupi_sebi_doniraj_drugom`.`Trenutne_cene` (
  `Cena` INT NOT NULL,
  `Licitacija_idLicitacija` INT NOT NULL AUTO_INCREMENT,
  `Korisnik_idKorisnik` INT NULL,
  `Licitator` VARCHAR(45) NULL DEFAULT 'null',
  INDEX `fk_Trenutne_cene_Licitacija1_idx` (`Licitacija_idLicitacija` ASC) ,
  INDEX `fk_Trenutne_cene_Korisnik1_idx` (`Korisnik_idKorisnik` ASC) ,
...
CREATE TABLE IF NOT EXISTS `kupi_sebi_doniraj_drugom`.`Recenzija` (
  ...
  INDEX `fk_Recenzija_Korisnik1_idx` (`Korisnik_idKorisnik` ASC) ,
...
CREATE TABLE IF NOT EXISTS `kupi_sebi_doniraj_drugom`.`Uplata` (
  ...
  `svrha uplate` INT NULL,
  `primalac` INT NOT NULL,
  PRIMARY KEY (`idUplata`),
  INDEX `fk_Uplata_Licitacija1_idx` (`svrha uplate` ASC) ,
  INDEX `fk_Uplata_Fondacija1_idx` (`primalac` ASC) ,
